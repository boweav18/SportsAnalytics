...
ui = fluidPage(
  ...
  checkboxInput("header", "Is there a Header?"),
  fileInput("file", "Upload New Data"),
  h5("Max file size 5 MB"),
...
server = function(input, output, session) {
  ...
  ###use js function refresh to refresh the rshiny page
  ...
  datasetInput = reactive({
    ...
    ## IF OFFENSIVE INDIVIDUAL ##
      ...
      if (length(input$file) > 0){
        new_file = input$file
        new_data = read.csv(new_file$datapath, sep = input$sep,header = input$header)
        master = rbind(master,new_data)
        write.csv(master,"mastertotal.csv")
      }
  ...
  #read_csv(new_data$datapath, sep = input$sep, header = input$header)
  
  #mastertotal_new <- rbind(mastertotal, new_data)
  
  output$mastertotal_new <- renderTable({
      ...
      read.csv(new_data$datapath, header = input$header, sep = input$sep)
